...
func profileRuleToParagraphStyle(pr *templateprofile.StyleRule, defaults paragraphStyle) paragraphStyle {
	...
	if pr.ComplexSizeHalfPt != "" {
		ps.complexSize = pr.ComplexSizeHalfPt
	}
	if pr.BoldSet {
		ps.bold = pr.Bold
		ps.boldSet = true
	...
	if pr.KeepNextSet {
		ps.keepNext, ps.keepNextSet = pr.KeepNext, true
	}
	if pr.KeepLinesSet {
		ps.keepLines, ps.keepLinesSet = pr.KeepLines, true
	}
	if pr.WidowControlSet {
		ps.widowControl, ps.widowControlSet = pr.WidowControl, true
	}
	if pr.Alignment != "" {
...
func chapterNumberFromHeading(text string) (string, bool) {
	...
	if !isLikelyNumberedHeading(trimmed, heading1Pattern) || isLikelyNumberedHeading(trimmed, heading2Pattern) {
		return "", false
	}
...
// isLikelyNumberedHeading keeps numeric prose such as "2016 年的数据表明"
// out of the heading formatter. The actual title format still comes from the
// template; this only rejects sentence-like numeric prefixes.
func isLikelyNumberedHeading(text string, pattern *regexp.Regexp) bool {
	if !pattern.MatchString(text) {
		return false
	}
	prefix := regexp.MustCompile(`^(\d+(?:\.\d+)*)\s+`).FindStringSubmatch(text)
	if len(prefix) != 2 {
		return false
	}
	if !strings.Contains(prefix[1], ".") {
		if value, err := strconv.Atoi(prefix[1]); err != nil || value > 99 {
			return false
		}
	}
	title := strings.TrimSpace(strings.TrimPrefix(text, prefix[0]))
	if title == "" || len([]rune(title)) > 80 || strings.ContainsAny(title, "。！？；;，,") {
		return false
	}
	return !strings.HasPrefix(title, "年") && !strings.HasPrefix(title, "月") && !strings.HasPrefix(title, "日") && !strings.HasPrefix(title, "天")
}
...
func figureCaptionNameFromContext(content string, matches [][]int, index int) string {
	for i := index - 1; i >= 0; i-- {
		...
		if isLikelyNumberedHeading(text, heading1Pattern) || isLikelyNumberedHeading(text, heading2Pattern) || isLikelyNumberedHeading(text, heading3Pattern) || isLikelyNumberedHeading(text, heading4Pattern) {
...
func styleForParagraph(text string, section *string) (paragraphStyle, bool) {
	...
	case isLikelyNumberedHeading(trimmed, heading4Pattern):
		*section = "body"
		return heading4Style(), true
	case isLikelyNumberedHeading(trimmed, heading3Pattern):
		*section = "body"
		return heading3Style(), true
	case isLikelyNumberedHeading(trimmed, heading2Pattern):
		*section = "body"
		return heading2Style(), true
	case isLikelyNumberedHeading(trimmed, heading1Pattern):
		*section = "body"
...
func applyParagraphStyle(paragraph string, style paragraphStyle) string {
	...
	// Word allows a paragraph's pPr/rPr to provide the default character
	// formatting for every run. Updating only the runs leaves that inherited
	// layer stale (and causes different readers to show different sizes/fonts),
	// so keep it in sync with the selected template rule as well.
	paragraph = paragraphPropertiesPattern.ReplaceAllStringFunc(paragraph, func(properties string) string {
		return pPrRunPropertiesPattern.ReplaceAllStringFunc(properties, func(existing string) string {
			patched, changed := ooxmlpatch.ApplyRunProperties("<w:r>"+existing+"<w:t></w:t></w:r>", paragraphStyleToRunPatchSpec(style))
			if !changed {
				return existing
			}
			if start := strings.Index(patched, "<w:rPr>"); start >= 0 {
				if end := strings.Index(patched[start:], "</w:rPr>"); end >= 0 {
					return patched[start : start+end+len("</w:rPr>")]
				}
			}
			return existing
		})
...
func paragraphStyleToPatchSpec(style paragraphStyle) ooxmlpatch.ParagraphPropertiesSpec {
	spec := ooxmlpatch.ParagraphPropertiesSpec{
		...
		KeepNextSet:       style.keepNextSet,
		KeepLines:         style.keepLines,
		KeepLinesSet:      style.keepLinesSet,
		WidowControl:      style.widowControl,
		WidowControlSet:   style.widowControlSet,
	}
...
func paragraphStyleToRunPatchSpec(style paragraphStyle) ooxmlpatch.RunPropertiesSpec {
	...
	return ooxmlpatch.RunPropertiesSpec{
		...
		ComplexSizeHalfPts: complexSize,
		Bold:               style.bold,
...
func buildRunProperties(style paragraphStyle) string {
	...
	if style.fontSize != "" {
		builder.WriteString(fmt.Sprintf(`<w:sz w:val="%s"/>`, style.fontSize))
	}
	if style.complexSize != "" {
		builder.WriteString(fmt.Sprintf(`<w:szCs w:val="%s"/>`, style.complexSize))
	}
	return builder.String()
}
