...
func (h *PaymentHandler) CreatePayment(c *gin.Context) {
	...
	validPaymentTypes := map[string][]string{
		"wechat": {"native", "jsapi"},
		"alipay": {"precreate", "wap", "page", "auto"},
	}
	...
	if err != nil {
		msg := err.Error()
		// 配置/业务类错误返回 400，便于前端展示原因，避免一律 500
		if strings.Contains(msg, "未配置") || strings.Contains(msg, "alipay precreate failed") ||
			strings.Contains(msg, "order is not in pending") || strings.Contains(msg, "order has expired") ||
			strings.Contains(msg, "failed to create wechat") || strings.Contains(msg, "incomplete") {
			utils.BadRequest(c, msg)
			return
		}
		utils.InternalServerError(c, msg)
		return
	}
...
// 支付宝要求响应 body 为纯文本 success / fail
func (h *PaymentHandler) HandleAlipayCallback(c *gin.Context) {
	if err := c.Request.ParseForm(); err != nil {
		log.Printf("[AlipayCallback] parse form: %v", err)
		c.String(200, "fail")
		return
	}
	...
	if err := h.paymentService.HandleAlipayCallback(params); err != nil {
		log.Printf("[AlipayCallback] %v", err)
		c.String(200, "fail")
		return
	}
